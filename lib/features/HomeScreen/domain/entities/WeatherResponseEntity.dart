...
class WeatherResponseEntity {
  WeatherResponseEntity({
    ...
    features = processWeatherFeatures();
  }
  ...
  // Process and extract features from the weather response
  List<int> processWeatherFeatures() {
    List<int> extractedFeatures = [];

    // Extract weather outlook (e.g., sunny, rainy, overcast)
    String? conditionText = forecast?.forecastday?[0].day?.condition?.text?.toLowerCase();
    if (conditionText != null) {
      if (conditionText.contains('sunny')) {
        extractedFeatures.add(1); // Sunny
        extractedFeatures.add(0); // Not rainy
      } else if (conditionText.contains('rain') || conditionText.contains('drizzle')) {
        extractedFeatures.add(0); // Not sunny
        extractedFeatures.add(1); // Rainy
      } else {
        // If it's overcast, misty, or anything else
        extractedFeatures.add(0); // Not sunny
        extractedFeatures.add(0); // Not rainy
      }
    } else {
      extractedFeatures.add(0); // Default to not sunny
      extractedFeatures.add(0); // Default to not rainy
    }

    // Extract temperature (hot, mild, cold)
    double? tempC = current?.tempC;
    if (tempC != null) {
      if (tempC >= 30) {
        extractedFeatures.add(1); // Hot
        extractedFeatures.add(0); // Not mild
      } else if (tempC >= 20 && tempC < 30) {
        extractedFeatures.add(0); // Not hot
        extractedFeatures.add(1); // Mild
      } else {
        extractedFeatures.add(0); // Not hot
        extractedFeatures.add(0); // Not mild (cold)
      }
    } else {
      extractedFeatures.add(0); // Default to not hot
      extractedFeatures.add(0); // Default to not mild
    }

    // Extract humidity (normal or high)
    int? humidity = current?.humidity;
    if (humidity != null) {
      if (humidity < 60) {
        extractedFeatures.add(1); // Normal humidity
      } else {
        extractedFeatures.add(0); // High humidity
      }
    } else {
      extractedFeatures.add(0); // Default to high humidity
    }
...
/// astro : {"sunrise":"05:54 AM","sunset":"05:35 PM","moonrise":"09:59 AM","moonset":"08:11 PM","moon_phase":"Waxing Crescent","moon_illumination":15,"is_moon_up":0,"is_sun_up":0}
